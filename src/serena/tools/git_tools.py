...
class GitLogTool(_GitTool):
    ...
    def apply(self, limit: int = 20, ref: str | None = None) -> str:
...
class GitDiffTool(_GitTool):
    ...
    def apply(self, staged: bool = False, paths: list[str] | None = None) -> str:
...
class GitBranchTool(_GitTool, ToolMarkerCanEdit):
    ...
    def apply(self, action: str, name: str | None = None, start_point: str | None = None) -> str:
        ...
        """
        if action == "list":
            return self._run_git(["branch", "--verbose", "--no-abbrev"])
...
class GitCommitTool(_GitTool, ToolMarkerCanEdit):
    ...
    def apply(self, message: str, paths: list[str]) -> str:
