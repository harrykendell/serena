...
def test_job_status_wait_for_completed_ignores_output_until_terminal(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    backend = FakeJobBackend()
    manager = _manager(tmp_path, backend)
    project = MagicMock(project_root=str(tmp_path), project_name="demo")
    agent = _job_tool_agent(project)
    ...
    def fake_sleep(_: float) -> None:
        ...
        sleep_calls += 1
        if sleep_calls == 1:
            backend.output[record.job_id].append("still working")
        elif sleep_calls == 2:
            backend.running.discard(record.job_id)
            manager._store.update(
                record.job_id,
                status=JobStatus.COMPLETED,
                finished_at="2026-09-07T22:00:00+00:00",
                return_code=0,
    ...
    assert status["output"] == "still working"
    assert status["next_cursor"] == "1"
