...
def test_language_server_process_survives_a_short_lived_calling_thread() -> None:
    ...
    a language server can be started from Serena's short-lived runtime-initialisation thread. The
    launcher must therefore establish PDEATHSIG relative to the long-lived Serena process rather than
    the transient caller, while also avoiding Python preexec callbacks in the forked child.
    """
